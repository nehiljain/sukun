...
from release_manager.config import (
    GITHUB_CLIENT_ID,
...
from releases.models import AppUser, SourceCodeAccount, SourceCodeProvider
...
def auth(request, provider="github"):
    """
    Check if the user is authenticated with GitHub.
    If not, redirect to GitHub for authentication.
    If yes, check the validity of the access token.
    If not valid, regenerate using refresh token.
    ...
    if not request.user.is_authenticated:
        logger.info("User is not authenticated, redirecting to GitHub...")
        scope = "user repo"
        random_string = uuid.uuid4().hex.upper()[0:15]
        url = f"https://github.com/login/oauth/authorize?response_type=code&scope={scope}&redirect_uri={GITHUB_REDIRECT_URI}&client_id={GITHUB_CLIENT_ID}&state={random_string}"
        return redirect(url)

    try:
        logger.info("User is authenticated, checking token validity...")
        app_user = request.user.appuser
        source_code_provider = SourceCodeProvider.objects.get(alias="github")
        source_code_account = SourceCodeAccount.objects.get(
            app_user=app_user, service_provider=source_code_provider
        )

        # Check token validity
        headers = {"Authorization": f"token {source_code_account.info['access_token']}"}
        response = requests.get("https://api.github.com/user", headers=headers)

        if response.status_code != status.HTTP_200_OK:
            logger.info("Token is invalid, trying to refresh...")
            # Token is invalid, try to refresh
            refresh_data = {
                "refresh_token": source_code_account.info["refresh_token"],
            ...
            if refresh_response.status_code == status.HTTP_200_OK:
                ...
                source_code_account.info["access_token"] = new_tokens["access_token"]
                source_code_account.info["refresh_token"] = new_tokens.get(
                    "refresh_token", source_code_account.info["refresh_token"]
                )
                logger.info(source_code_account.info)
                source_code_account.save()
            else:
                # If refresh fails, redirect to re-authenticate
                logger.info("Token refresh failed, redirecting to re-authenticate")
                return redirect("auth")

        # User is authenticated and token is valid
        logger.info("Token is valid, logging in user and redirecting to dashboard")
        login(request, request.user)
        return redirect("/dashboard")

    except (AppUser.DoesNotExist, SourceCodeAccount.DoesNotExist) as e:
        logger.error(e)
        # If user doesn't have GitHub account linked, redirect to authenticate
        logger.error(
            "User doesn't have GitHub account linked, redirecting to authenticate"
        )
        return redirect("/auth")
...
def callback(request):
    ...
    if "access_token" in res_obj:
        ...
        service_account_response = requests.get(
            "https://api.github.com/user",
            headers={"Authorization": f"Bearer {access_token}"},
        )
        logger.info(service_account_response.json())
        username = service_account_response.json()["login"]
        name = service_account_response.json()["name"]
        profile_url = service_account_response.json()["avatar_url"]
        try:
            user = User.objects.get(username=username)
        except User.DoesNotExist:
            logger.info(
                f"Creating new user with username: {username}, email: {username}@github.com"
            )
            try:
                with transaction.atomic():
                    ...
                    logger.info(f"User has active organization: {app_user.active_org}")
                    ensure_user_is_stripe_customer(app_user)
            except Exception as e:
                logger.error(f"Error creating user and app_user: {str(e)}")
                return redirect("/#?error=user_creation_failed")

        app_user = AppUser.objects.get(user=user)
        source_code_provider = SourceCodeProvider.objects.get(alias="github")

        source_code_account, created = SourceCodeAccount.objects.get_or_create(
            app_user=app_user,
            service_provider=source_code_provider,
            defaults={
                "info": {
                    "access_token": res_obj["access_token"],
        ...
        if not created:
            source_code_account.info["access_token"] = res_obj["access_token"]
            source_code_account.info["refresh_token"] = res_obj["refresh_token"]
            source_code_account.save()
        # token, created = Token.objects.get_or_create(user=user)
        login(request, user)

        # After login, check if user needs to complete onboarding
...
def google_callback(request):
    ...
    try:
        ...
        source_code_provider = SourceCodeProvider.objects.get_or_create(
            alias="google", defaults={"name": "Google"}
        )[0]
        ...
        if not created:
            source_code_account.info.update(
                {
                    "access_token": token_data["access_token"],
                    "id_token": token_data["id_token"],
                }
            ...
            if "refresh_token" in token_data:
                source_code_account.info["refresh_token"] = token_data["refresh_token"]
            source_code_account.save()
        ...
        if not app_user.is_email_verified:
            try:
                ...
                from releases.views_users import send_verification_email
