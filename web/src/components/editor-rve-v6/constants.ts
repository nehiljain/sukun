...
// Button click animation timing constants (in frames, at 30fps)
export const BUTTON_CLICK_TIMING = {
  ENTRY: 15, // 0.5s
  MOVEMENT: 15, // 0.5s (concurrent with entry)
  CLICK: 5, // 0.167s
  STATE: 10, // 0.333s
} as const;

// Helper to calculate total duration
export const calculateButtonClickDuration = (
  timing: typeof BUTTON_CLICK_TIMING,
) => {
  return timing.ENTRY + timing.CLICK + timing.STATE;
};
