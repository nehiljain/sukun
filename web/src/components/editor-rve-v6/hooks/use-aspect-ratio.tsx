...
import { useVideoProjectState } from "./use-video-project-state";
 ...
 * @param videoProjectId - The ID of the video project (optional)
...
export const useAspectRatio = (
  ...
  const { saveProject } = useVideoProjectState(videoProjectId);
  ...
  const handleAspectRatioChange = useCallback(
    async (newRatio: AspectRatio) => {
      ...
      // Save the new aspect ratio to the project state
      if (videoProjectId) {
        await saveProject({
    ...
    [onRatioChange, videoProjectId, saveProject],
  );

  // Initialize with the provided initial ratio
  useEffect(() => {
